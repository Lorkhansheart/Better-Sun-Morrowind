...
#define sunrayocclude 0.50       // How much sun rays will 'overwrite' original image
...
#define sundiscocclude 0.50      // How much the sun disc will 'overwrite' original image
#define lowsuncol float3(1.0, 0.46, 0.56)  // sun disc color at its lowest
#define highsuncol float3(0.43, 0.492, 0.144)   // sun disc color at its highest
...
static const float3 suncoldisc = lerp(lowsuncol, highsuncol, sunpos.z) * saturate(suncol / max(suncol.r, max(suncol.g, suncol.b)) * (1 - sundiscdesaturate) + float3(sundiscdesaturate, sundiscdesaturate, sundiscdesaturate));
